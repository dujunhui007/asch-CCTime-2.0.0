...
function AschServer(url) {
  ...
  this.checkServerStatus = () => {
    ...
    axios
      .get(server.serverUrl + '/api/system/', { timeout: CHECK_SERVER_TIMEOUT })
  ...
  this.updateStatus = responseHeaders => {
  ...
  this.isServerAvalible = ignorePending => {
    // 版本不低于1.3.4,且是健康的节点
    return (
      (this.state === 'success' || (ignorePending && this.state === 'pending')) &&
      this.versionNotLessThan('1.3.4') &&
      this.isHealthy()
    )
  }
...
AschServer.compareServer = (server1, server2) => {
...
function getPeers(seedServerUrl, onSuccess, onFailed) {
  ...
  axios.get(seedServerUrl + '/api/peers?limit=100').success(
    (data, status, headers) => {
      if (!data.success) {
        onFailed()
        return
      ...
      let nodes = selectServerNodes(data.peers) || []
      nodes.forEach(node => {
...
this.dynamicServers = originalServerUrl => {
  this._isStaticServer = false
...
this.changeServer = randomSelect => {
  if (currentServer !== null) {
    var avalibleServers = this.getSortedAvalibleServers()
